...
def do_val_models(save_checkpoint_path,ds_val):
    ...
    best_ckpt_name="Best_Model_"+best_ckpt.split("/")[-1].split("\\")[-1]
...
def do_eval(dataset=None, network=None, num_class=None, load_checkpoint_path="",do_plot=False,do_train=False):
    ...
    print_result["ACC"]=metrics.accuracy_score(true_labels,pred_labels,average='micro')
...
def do_predict(seq_len=2048, network=None, num_class=2, load_checkpoint_path="",tokenizer=None):
    ...
    ds_predict = pd.read_csv(args_opt.data_url).to_dict("records")  # predict csv must have["id","seq"] or ["id_0","seq_0","id_1","seq_1"]
    ...
    for data in tqdm(ds_predict):
        ...
        if args_opt.return_sequence == True or args_opt.return_csv == True:
            data["dense_0"] = logits.asnumpy()[0]
            data["dense_1"] = logits.asnumpy()[1]
            # data["dense_2"] = logits.asnumpy()[2]
